"""
End-to-end pipeline: NCAA Women's Volleyball match prediction
  1. Load & combine raw source files
  2. Clean opponent names
  3. Filter to paired (symmetric) matches
  4. Build cumulative features
  5. Engineer advanced features (Elo, form, per-set stats, etc.)
  6. Train XGBoost with walk-forward validation
"""
...
for i in range(1, len(seasons)):
    ...
    print(
        f"  Optimal threshold (val): {best_t:.2f}  |  val acc: {best_acc:.4f}"
    )

    preds = (probs >= best_t).astype(int)
    season_acc = (preds == y_test.values).mean()
    print(f"  Test accuracy: {season_acc:.4f} ({len(y_test)} rows)")
...
if len(all_true) == 0:
    print("No predictions generated — check feature cleaning.")
else:
    print(classification_report(all_true, all_preds))
    print("ROC AUC:", round(roc_auc_score(all_true, all_probs), 4))

    # Mean feature importance across folds is more stable than a single fold.
    importance_df = pd.DataFrame(all_fold_importances, columns=X_tr.columns)
    importance = importance_df.mean(axis=0).sort_values(ascending=False)
    print("\nTop 20 Mean Feature Importances:")
    ...
    def feature_family(name):
        n = str(name).lower()
        if "elo" in n or "sos" in n:
            return "Elo / Strength"
        if "h2h" in n:
            return "Head-to-Head History"
        if "form" in n or "last" in n or "streak" in n or "win_rate" in n:
            return "Form / Momentum"
        if (
            "hit_pct" in n or "kills" in n or "errors" in n
            or "aces" in n or "assists" in n or "digs" in n
            or "set_" in n or "sets_" in n
        ):
    ...
    family_colors = {
        "Elo / Strength": "#3776b6",
        "Head-to-Head History": "#6cab44",
        "Form / Momentum": "#ea7b31",
        "Efficiency Metrics": "#f2bd00",
        "Other Features": "#9a9a9a",
    }
    bar_colors = [family_colors[feature_family(f)] for f in top_importance.index]

    fig, ax = plt.subplots(figsize=(12, 8))
    bars = ax.barh(top_importance.index, top_importance.values * 100, color=bar_colors)

    max_val = float((top_importance.values * 100).max()) if len(top_importance) else 0.0
    pad = max(0.12, max_val * 0.01)
    for bar, val in zip(bars, top_importance.values * 100):
        ax.text(val + pad, bar.get_y() + bar.get_height() / 2, f"{val:.2f}%", va="center", fontsize=10)

    ax.grid(axis="x", linestyle="--", alpha=0.35)
    ax.set_axisbelow(True)
    ax.set_xlabel("Mean Feature Importance (%) — averaged across all folds", fontsize=12)
    ax.set_ylabel("Feature", fontsize=12)
    ax.set_title("Top 20 Feature Importances\nXGBoost Model — NCAA Volleyball Match Prediction", fontsize=16, fontweight="bold")
    ax.set_xlim(0, max_val + pad * 10)

    legend_handles = [
        mpatches.Patch(color=family_colors[k], label=k)
        for k in [
            "Elo / Strength",
    ...
    plt.tight_layout()
    plot_path = "SEM8/Grad Project/new_feature_importance.png"
...
# # Uncomment and run on a single season split to find better params,
# # then paste the best_params into the XGBClassifier above.
# # # =============================================================================

# import optuna
# optuna.logging.set_verbosity(optuna.logging.WARNING)

# # Use last training fold for the search
# train_opt = df[df["season"].isin(seasons[:-1])]
# X_opt = train_opt[features + loc_cols]
# y_opt = train_opt["win"]
# X_tr_o, X_val_o, y_tr_o, y_val_o = train_test_split(X_opt, y_opt, test_size=0.15, random_state=42)
...
#     }
#     m = XGBClassifier(**params, eval_metric="logloss", random_state=42, n_jobs=-1)
#     m.fit(X_tr_o, y_tr_o, eval_set=[(X_val_o, y_val_o)],
#         verbose=False)
#     preds = m.predict(X_val_o)
#     return (preds == y_val_o.values).mean()
